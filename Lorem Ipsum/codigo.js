const countP = document.querySelector(".article-center input");
const btnGenerate = document.querySelector(".article-center button");
const newText = document.querySelector(".article-center-generate");
...
function generadorText(randomValue){
    return `<p>${text[randomValue]}</p>`
}
...
btnGenerate.addEventListener("click", ()=>{
    ...
    let valueGenerate = countP.value;

    for (i = 0; i < valueGenerate; i++){
        let randomValue = Math.floor(Math.random(numbers) * (numbers.length));
        
        newText.innerHTML += generadorText(randomValue);
    }
});
